...
class CommandSearchWikipedia:
    ...
    def search(self, keyword, search_term=None):
        ...
        if condition is None:
            return {"status": "CONDITION_NOT_FOUND", "message": "Comando de pesquisa na Wikipedia não encontrado."}

        question, response = condition
        if search_term is None or not str(search_term).strip():
            return {"status": "WAITING_FOR_SEARCH_TERM", "question": question}

        term = str(search_term).strip()
        article = quote(term.replace(" ", "_"), safe="_")
